...
def create_access_token(subject: Union[str, Any], token_version: int = 1, expires_delta: Optional[timedelta] = None) -> str:
    if expires_delta:
        ...
        expire = datetime.utcnow() + timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES)  # Use config setting
    
    to_encode = {"exp": expire, "sub": str(subject), "ver": token_version, "type": "access"}
    encoded_jwt = jwt.encode(to_encode, settings.JWT_SECRET, algorithm=settings.JWT_ALGORITHM)
    return encoded_jwt

def create_refresh_token(subject: Union[str, Any], token_version: int = 1, expires_delta: Optional[timedelta] = None) -> str:
    if expires_delta:
        expire = datetime.utcnow() + expires_delta
    else:
        expire = datetime.utcnow() + timedelta(days=7)  # 7 days default for long-lived refresh
