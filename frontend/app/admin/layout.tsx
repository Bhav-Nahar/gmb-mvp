...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const onLpseo = !!pathname?.startsWith('/admin/lpseo') || !!pathname?.startsWith('/admin/cseo') || !!pathname?.startsWith('/admin/cityseo')
      ...
      <header className="h-16 sticky top-0 z-30 border-b border-border bg-card px-6 flex items-center justify-between">
        <div className="flex items-center gap-5">
          ...
          <nav className="hidden sm:flex items-center gap-1">
            <Link href="/admin" className={navCls(!onAudit && !onHolidays && !onPseo && !onLpseo && !onLeads)}>Accounts</Link>
            ...
            <Link href="/admin/lpseo" className={navCls(onLpseo)}>Local SEO</Link>
            <Link href="/admin/leads" className={navCls(onLeads)}>Leads</Link>
          </nav>
        </div>
