...
export const FEATURES: Feature[] = [
  ...
  {
    slug: 'gbp-change-detection',
    name: 'Unauthorised Change Alerts',
    icon: 'History',
    badge: 'Profile protection',
    metaTitle: 'Google Business Profile Change Detection & Alerts | Pinzo',
    metaDescription: 'Know when your Google Business Profile changes — by staff, an old agency, or Google itself. Pinzo checks daily, shows what changed, and lets you accept or undo Google’s edits.',
    h1: 'Know the moment your listing changes',
    heroSub: 'Your Google Business Profile can be edited by anyone with access — and by Google itself, from a stranger’s suggested edit. Pinzo checks every location, shows you exactly what changed, and lets you push your version back.',
    definition: 'Google Business Profile change detection compares your live listing against its last known state and reports every difference — phone, address, hours, categories, description, attributes. It catches edits made outside your control, including changes Google applies on its own from Maps user suggestions.',
    howItWorks: [
      { title: 'We snapshot every field', detail: 'Each sync records your listing exactly as Google returns it — phone, hours, address, categories, description and attributes.' },
      { title: 'We compare, daily', detail: 'The next check diffs the live listing against that snapshot and reports anything that moved, with the old and new value side by side.' },
      { title: 'We ask Google directly', detail: 'Separately, we ask Google whether it has overridden your listing from its own sources — the one thing a normal sync can never see.' },
      { title: 'You accept or undo', detail: 'For a change Google made, keep its version or restore yours in one click. We publish the fix back to Google for you.' },
    ],
    benefits: [
      { title: 'Catch a wrong phone number', detail: 'A changed number can quietly cost you calls for weeks. You will see it the next day instead.' },
      { title: 'See Google’s own edits', detail: 'Google can change your hours or category from public suggestions without telling you. Those show up here.' },
      { title: 'Undo in one click', detail: 'Restore your own value and we push it back to Google through the same approval flow as any other edit.' },
      { title: 'Every location, one feed', detail: 'One page across all branches, filterable by location and by what caused the change.' },
    ],
    faqs: [
      { q: 'Will it tell me who made the change?', a: 'No — and neither will any other tool. Google’s API does not report who edited a listing. Pinzo tells you what changed, what it was before, and when we noticed. If several people have access, that narrows it down but cannot name them.' },
      { q: 'How quickly will I know?', a: 'Edits to your listing are picked up on the daily sync, so within a day. Changes Google itself applies are checked weekly, because that check costs an extra call to Google for every location.' },
      { q: 'Can it undo a change automatically?', a: 'No, and deliberately so — an automatic revert would fight a legitimate edit by your own staff. You decide, then Pinzo publishes it.' },
      { q: 'What if a change is made and reversed before you check?', a: 'It will not appear. Detection compares snapshots rather than watching a live event stream, because Google offers no such stream. Anything that changes and changes back between two checks is invisible.' },
      { q: 'Does it cover attributes too?', a: 'Yes — things like parking, payment methods, accessibility and delivery are compared one by one, so you see exactly which one moved rather than a wall of JSON.' },
    ],
    plan: 'Included on Basic and Pro.',
    related: ['multi-location-gbp-management', 'gbp-audit', 'local-insights'],
  },
  {
